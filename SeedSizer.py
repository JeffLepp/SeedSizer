...
FILTER = 0.3                                                                    # This is basically used saying we won't accept anything < .1 mm^2 in size

#################################################################################################################################################################################################

path = 'Data/Calena-18.tif'                                                      # Change this to analyse a different file in Data
...
def Run(filename):
    ...
    df["area_mm2"] = df["area"] / PP_SQMM                                           # Convert these connected components to mm^2 since we know pixel is 1/1200 ofan inch
    mean_area = df ["area_mm2"].mean()                                              # Finding average size of connected components


    ### Statistical Analysis ###

    df["abs_dev"] = (df["area_mm2"] - mean_area).abs()                              # Finding absolute deviation for area
    df_filtered = df.nsmallest(int(len(df) * .8), "abs_dev")                        # Selecting 80% of data w/ smallest deviantion from the mean

    df_filtered = df_filtered.drop(columns="abs_dev")   
    ...
    print(f"Total number of filtered seeds: {len(df_filtered)}")                    # Filtered = all connected components (seeds) of area > .1 mm^2 & is within 80% of data that is closest to mean
    print(f"Average seed size is: {mean_area}")
    print()

    '''
    print(df[["label", "area_mm2"]].head())                                         # Gives some ideas on the mm^2 you're getting for unfiltered
    print(df_filtered[["label", "area_mm2"]].head())                                # Gives some ideas on the mm^2 you're getting for filtered

    '''
    ### Histogram ###
    
    '''
    sns.histplot(df_filtered["area_mm2"], bins=50)                                    # Histogram for filtered seeds (basically 1.25 mm^2 - 2.5 mm^2)
    plt.title("80% Central Seed Area Distribution")
    plt.xlabel("Area (mm^2)")
    plt.show()

    
    sns.histplot(df["area_mm2"], bins=50)                                             # Histogram for unfiltered seeds (basically 0.00 mm^2 - 2.5 mm^2)
    plt.title("Seed Area Distribution")
    plt.xlabel("Area (mm^2)")
    plt.show()
    '''

    return len(df), len(df_filtered), mean_area
